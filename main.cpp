...
clase_t Knn (const vector<pair<double,clase_t> >& vecNormas, uint k) {//las labels podrian no ser un uint pero lo dejo asi en un principio
...
vector<uint> vectorDeKnns (const vector<vector<double> >& trainX, const vector<clase_t>& labelsX, const vector<vector<double> >& testY, uint k){
	...
	vector<uint> res (n);
	for (uint i = 0; i < n; i++) {
        vector<pair<double,clase_t> > vec_dist = vector_de_distancias(trainX, labelsX, testY[i]);
		res[i] = Knn(vec_dist, k);
	}
	return res;
}
    ...
    for(unsigned char fold = 0; fold < cant_folds; ++fold){
        ...
        for(unsigned short int test = 0; test < cant_para_test; ++test){
            ...
            for(uint k = 0; k < cant_ks; ++k){
                predicciones[k][test] = Knn(vec_dist, 1+4*k);
...
*/
        ...
        //cargarTest_aleat(dataSetTest, false);		<<<<<<<<<<< Este es para las imagenes aleatorias
        //------- cargamos los datos de uno de los tests en la funcion cargarTest esta la explicacion de que hace-------------------//

		//testDistorted(*dataSetTest,*labelsTest,5);
        //kfold_para_imagenes_aleatorios(*dataSetTest, 80, 20, 10);
