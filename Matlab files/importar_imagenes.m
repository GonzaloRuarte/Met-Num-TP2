function [ matriz_imagenes ] = importar_imagenes( tp2_folder )
%imp_im importa las imagenes a una matriz
...
%   sistema operativo ubuntu.
    ...
    for suj_n = 2:41
        ...
        for im_n = 1:10
            image_number = char(string(im_n)); %combierto el numero im_n en un string de numeros, y despues en un arreglo de chars.
            X = importdata([tp2_folder, 'ImagenesCaras/s', subject_number, '/', image_number, '.pgm']);
            X = reshape(X',1,[]);
            matriz_imagenes = [matriz_imagenes; X];
        end
    end
